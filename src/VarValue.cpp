...
std::string VarStorage::Name()
{
	...
	case Register:
		return reg.Name();
	case Local:
		return elitf_format::format("local_{:x}", -offset);
	case Argument:
		return elitf_format::format("arg_{}", idx);
	case Static:
		return elitf_format::format("static_{:x}", offset);
	case Pool:
		return elitf_format::format("PP_{:x}", offset);
	case Thread:
		return elitf_format::format("THR_{:x}", offset);
	case SmallImm:
