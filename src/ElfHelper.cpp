...
LibAppInfo ElfHelper::findSnapshots(const uint8_t* elf)
{
        ...
        return LibAppInfo{.lib = elf, .vm_snapshot_data = nullptr, .vm_snapshot_instructions = nullptr, .isolate_snapshot_data = snapshot_data, .isolate_snapshot_instructions = snapshot_text};
#else
